    ...
    function displayTeacher($status){
        ...
        if($result->num_rows > 0){
            while($row = $result->fetch_assoc()){
                ...
                echo "<td>" . $row["user_info_id"] . "</td><td>".$row["personal_id"]."</td><td>" . $row["first_name"] ."</td><td>". $row["last_name"] ."</td><td>". $row["gender"]. "</td><td>" . $row["user_level_description"]. "</td><td><b><span class='".$statusColor."'>" . $row["status"] ."</b></span></td>";
                echo "<td><a href='#' class='add_class_btn text-primary' type='button' data-toggle='tooltip' title='Assign Class' data-id='".$row["user_info_id"]."'><i class='fa fa-plus'></i></a></td>";
                echo "<td><a href='#' class='add_module_btn text-warning' type='button' data-toggle='tooltip' title='Assign Module' data-id='".$row["user_info_id"]."'><i class='fa  fa-file-text-o></i></a></td>";
