    ...
    function displayAdmin($status){
        ...
        if($result->num_rows > 0){
            while($row = $result->fetch_assoc()){
                ...
                echo "<td>" . $row["user_info_id"] . "</td><td>" . $row["first_name"] ."</td><td>". $row["last_name"] ."</td><td>" . $row["user_level_description"]. "</td><td><b><span class='".$statusColor."'>" . $row["status"] ."</b></span></td>";
