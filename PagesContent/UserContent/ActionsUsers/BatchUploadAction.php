                ...
                if ($isIdvalid && $isValid) {
                    ...
                    try {
                        ...
                        if ($addNewUser->getLastError() === null) {
                            // if no error create a credentials
                            ...
                            try {
                                ...
                                if ($addNewCreds->getLastError() === null) {
                                    ...
                                    // !if contact adding is successful
                                    if ($addNewContact->getLastError() === null) {
                                        ...
                                        if ($addNewTeacher->getLastError() === null) {
                                            $responses[] = ['success' => "Successfully added new teacher"];
                                        } else {
                                            $responses[] = ['error' => "Error adding on teacher table"];
                                        }
                                    } else {
                                        $responses[] = ['error' => 'Error Adding Contact Info for' . $values['first_name'] . ' ' . $values['last_name'] . '!'];
                                    }
                                } else {
                                    $responses[] = ['error' => 'Error Adding Credentials for' . $values['first_name'] . ' ' . $values['last_name'] . '!'];
                                }
                            ...
                            $responses[] = ['error' => 'Error Adding user info!' . $values['first_name'] . ' ' . $values['last_name'] . '!'];
                        }
                    } catch (mysqli_sql_exception $e) {
                        // Handle any errors during insertion
                        $responses[] = ['error' => $e->getMessage()];
                    ...
                    $responses[] = ['error' => 'Error uploading file! Possible Duplicate'];
                }
            }
        } else {
            $responses[] = ['error' => 'Error uploading file!'];
        }
    } else {
        $responses[] = ['error' => 'Please upload a valid Excel file!'];
    }

    // Return the array of responses as a JSON response
