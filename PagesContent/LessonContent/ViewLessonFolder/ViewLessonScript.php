    ...
    function loadData() {
        $.ajax({
            ...
            success: function(response) {
                responseData = response; // Assume the backend returns data as an array
                loadPage(currentIndex);

                var responseDataJson = JSON.parse(response);
                if (responseDataJson.hasOwnProperty('error')) {
                    $('#errorAlert').text(responseDataJson.error);
                    $('#errorBanner').show();
                }
            },
            error: function() {
    ...
    function loadPage(index) {
        if (responseData) {
            ...
            if (fileType === 'jpg' || fileType === 'jpeg' || fileType === 'png' || fileType === 'gif') {
                ...
                content = '<audio controls><source src="' + currentItem + '" type="audio/' + fileType +
                    '">Your browser does not support the audio element.</audio>';
            } else if (fileType === 'mp4' || fileType === 'webm' || fileType === 'ogg') {
                content = '<video controls><source src="' + currentItem + '" type="video/' + fileType +
                    '">Your browser does not support the video element.</video>';
            } else if (fileType === 'pdf') {
                content = '<embed src="' + currentItem +
